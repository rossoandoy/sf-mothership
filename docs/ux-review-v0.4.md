...
**対応**: `useTabPanel` は各ツール完了ごとに `partial` 更新し、`TabPanelShell` は結果なし読み込み時に要約カード型スケルトン、結果あり読み込み時に部分結果 + pending 件数を表示する。
...
**対応**: `deriveExtensionStatus` / `useExtensionStatus` で `waitingForContext`、`staleContentScript`、`serviceWorkerError` を分離し、Service Worker error / waiting 時は `再取得` CTA、stale content script 時は Salesforce タブ再読み込み案内を主表示にする。
...
**対応**: `buildContextIdentity` で orgDomain / pageType / objectApiName / recordId / url を比較し、文脈変更時に `activeTab` を `instant` へ戻す。prefetch は未実装。
...
**対応**: `tabPanelCache` を `{ state, cachedAt }` に変更し、5分 TTL、error 非永続化、cache key への `context.url` 追加を実装。stale-while-revalidate は未実装。
...
- InstantSummary 狭幅時の情報量整理
- cache stale-while-revalidate
- fields / guide の遅延 prefetch
